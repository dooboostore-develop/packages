...
async function buildTarget(target, watch = false) {
  ...
  switch (target) {
    ...
    case 'umd-bundle':
      ...
      await performBuild({
        ...baseOptions,
        bundle: true, // Override to true
        external: ['canvas'], // Explicitly external
        entryPoints: [path.resolve(srcDir, 'bundle-entry.ts')],
        outfile: path.resolve(__dirname, 'dist', 'umd-bundle', 'dooboostore-lib-node.umd.js'),
        format: 'iife',
        globalName: 'dooboostoreLibNode',
    ...
    case 'esm-bundle':
      ...
      await performBuild({
        ...
        external: ['canvas'], // Explicitly external
        entryPoints: [path.resolve(srcDir, 'bundle-entry.ts')],
        outfile: path.resolve(__dirname, 'dist', 'esm-bundle', 'dooboostore-lib-node.esm.js'),
        format: 'esm',
