  ...
  try {
    ...
    files.forEach(file => {
      ...
      try {
        ...
        } else if (filepath.endsWith('.ts')) { // ONLY .ts files as entry points
...
const entryPoints = tsFiles.reduce((acc, file) => {
  const entryName = path.relative(srcDir, file).replace(/\.ts$/, ''); // Only strip .ts extension
  acc[entryName] = file;
  return acc;
}, {});
...
async function buildTarget(target, watch = false) {
  const baseOptions = {
    bundle: false, // Default to false for library builds
    sourcemap: true,
    target: 'es2020',
  ...
  switch (target) {
    ...
    case 'umd-bundle':
      ...
      await performBuild({
        ...baseOptions,
        bundle: true, // Override to true for bundle
