...
const config: Config = {
  ...
  targetElements: [
    DomRender.createComponent({ tagName:'details', type: User, noStrip:true, template: '<div>user${this.name}$</div>' }),
    DomRender.createComponent({ type: Profile, styles:'div{}', noStrip:true, template: '<div>profile<hr/> <input type="text" dr-value-link="@nearThis@.name" data-wow="${@nearThis@.name}$" >\n' +
