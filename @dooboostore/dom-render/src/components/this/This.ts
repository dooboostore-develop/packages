...
export default {
  this: (config?: DomRenderRunConfig) => {
    return RawSet.createComponentTargetElement({
      ...
      // template: '<dr-if value="${(!!@this@?.value) && (@this@?.if === null ? true : @this@.if)}$"><div dr-this="@this@.value" dr-detect-option-if="@this@?.value && @this@?.if" dr-option-strip="true" dr-on-create:arguments="@this@.createArguments" dr-on-create:callback="@this@.created($component)">#innerHTML#</div></dr-if>',
