...
export class EventManager {
  ...
  public changeVar(obj: any, elements: Set<Element> | Set<ChildNode>, varName?: string, config?: Config) {
    ...
    // forEach elements
    // DomRenderProxy set에서 하기때문에?? 우선지워둠 20250512
    // value-link event
    // EventManager.linkAttrs.forEach(linkInfo => {
    ...
    // RawSet CheckPoint에서 normalAttribute로 변경했음 20250512
    // attribute
    // this.procAttr(elements, EventManager.attrAttrName, (it, attribute) => {
    //   // it.setAttribute('dr-event-click', 'console.log("222222222")');
    //   let script = attribute;
    //   if (script) {
    //     script = 'return ' + script;
    //   }
    ...
    // 훔.. 살려야되나? DomRenderProxy set에서 하기때문에?? 우선지워둠 20250512
    ...
    //         })
    //       }));
    //       // console.log('-----change--normalAttribute---datadatadata',data)
    //       if (data ===null) {
    //         it.removeAttribute(k);
    //       } else {
    //         console.log('normalAttrMapAttrName setAttribute???', k, data)
    //         it.setAttribute(k, data);
    //       }
    //     }
    //   });
