...
describe('Append Tests', () => {
  ...
  test('should append cloned template.content multiple times', () => {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      if (itemText) itemText.textContent = `Item ${i}`;
      list?.append(clone);
    }
...
<body id="app" >
 <div id="target"></div>
    ...
    console.log(
      'Body childNodes after replaceChild:',
      Array.from(document.body.childNodes).map(n => `${n.nodeType}:${n.nodeName}:${n.textContent?.trim()}`)
    );
  ...
  test('should preserve text nodes when inserting between existing content', () => {
    ...
    console.log(
      'Initial nodes:',
      initialNodes.map(n => `${n.nodeType}:${n.nodeName}:${n.textContent?.trim()}`)
    );
    ...
    console.log(
      'Final nodes:',
      finalNodes.map(n => `${n.nodeType}:${n.nodeName}:${n.textContent?.trim()}`)
    );
