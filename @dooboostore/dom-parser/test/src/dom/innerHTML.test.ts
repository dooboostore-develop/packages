...
describe('innerHTML Tests', () => {
  ...
  test('should handle innerHTML setter and getter consistency', () => {
    ...
    console.log(
      'testDiv childNodes:',
      Array.from(testDiv.childNodes).map(n => `${n.nodeType}:${n.nodeName}:${n.textContent?.trim()}`)
    );
  ...
  test('should handle innerHTML with complex JavaScript expressions', () => {
    ...
    const changeSelectedAttr = drSelect.getAttribute('changeSelected') || drSelect.getAttribute('changeselected');
    ...
    const retrievedHTML = div.innerHTML;
    assert.ok(retrievedHTML.includes('<dr-select'), 'Retrieved HTML should contain dr-select tag');
    assert.ok(retrievedHTML.toLowerCase().includes('changeselected='), 'Retrieved HTML should contain changeselected attribute');
  ...
  test('should handle multiple complex JavaScript expressions', () => {
    ...
    const roundTripSelect = testContainer.querySelector('dr-select');
    const roundTripChangeSelected = roundTripSelect.getAttribute('changeSelected') || roundTripSelect.getAttribute('changeselected');
    assert.equal(roundTripChangeSelected, '${(data) => @this@?.updateItem?.(#item#.id, data)}$', 'Complex JavaScript should survive round-trip');
