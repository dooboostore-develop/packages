...
describe('TradingSimulator.simulate', () => {
  ...
  it('directionProbability — 매크로 있으면 13특성, 길이 어긋나면 폴백', () => {
    const data = mkCandles(Array.from({ length: 40 }, (_, i) => 100 + Math.sin(i) * 5 - i * 0.2));
    const base = C.directionProbability(data);
    const n = data.length;
    const flat = (v: number) => new Array(n).fill(v);
    const withMacro = C.directionProbability(data, { tnx: flat(4), wti: flat(70), spx: flat(5000) });
    assert.ok(withMacro >= 0 && withMacro <= 1);
    const fallback = C.directionProbability(data, { tnx: [1], wti: [1], spx: [1] });
    assert.equal(fallback, base, '길이 어긋나면 베이스와 동일해야');
  });
...
describe('TradingSimulator.simulate (real candles: candle.json)', () => {
  ...
  const config = C.findBestConfig(candles, { strategyRate: 0.5, marketRate: 0.5, indicators, tuning: { minEdge: 0 } });
