...
import { predictMacroLogOdds } from './boostModelMacro';
...
export namespace TradingSimulator {
  ...
  export interface FindBestOptions {
    ...
    /** 시장 외부요인 (TNX/WTI/SPX, candles 정렬). 있으면 13특성 매크로 모델 */
    macro?: MarketSeries;
  ...
  export const forecast = (
    ...
    const edge = Math.abs(directionProbability(candles, market) - 0.5) * 2;
  ...
  /** 시장 외부요인 — candles와 동일 길이·인덱스 정렬 (없으면 베이스 10특성 모델).
   *  값은 일자별 레벨 (국채10년, WTI, S&P500). 5일 변화율로 특성 변환. */
  ...
  /**
   * 5봉 후 상승확률 0~1 — 부스팅 10특성 (temp/export_trees.py 생산, thirds 1+2 학습).
   * 평균회귀형 + 우측면적(rightRatio) 하락 모멘텀. 기록 부족(30봉 미만) 시 0.5 중립.
   * 특성: [과거10수익합, 최근3모멘텀, 변동성, RSI대용−0.5, 거래량추세,
   *        MACDdiff, OBV기울기, volRatio, 장기MA이격, rightRatio] (temp/ds.py와 동일 정의).
   */
  export const directionProbability = (
    ...
    // rightRatio — 최근 20봉 중 후반 10봉 면적 비중 (바닥에 처박히면 하락 지속, 메모 #18)
    const w20 = closes.slice(i - 19, i + 1);
    const wMin = Math.min(...w20);
    const leftA = w20.slice(0, 10).reduce((s, x) => s + (x - wMin), 0) / closes[i];
    const rightA = w20.slice(10).reduce((s, x) => s + (x - wMin), 0) / closes[i];
    const rightRatio = rightA / Math.max(leftA + rightA, 1e-12);
    const f = [sumM, mom3, vol, rsi - 0.5, vtrend, md, ob, vr, (closes[i] - ma30) / closes[i], rightRatio];
    if (f.some(v => !Number.isFinite(v))) return 0.5;
    // 매크로 있으면 13특성 모델, 없으면 베이스 10특성
    if (market && market.tnx.length === n && market.wti.length === n && market.spx.length === n) {
      const chg = (xs: readonly number[]): number | null => {
        const a = xs[i - 5]; const b = xs[i];
        return a > 0 && Number.isFinite(a) && Number.isFinite(b) ? (b - a) / a : null;
      };
      const t = chg(market.tnx); const w = chg(market.wti); const s = chg(market.spx);
      if (t != null && w != null && s != null) {
        return 1 / (1 + Math.exp(-predictMacroLogOdds([...f, t, w, s])));
      }
    }
  ...
  export const findBestConfig = (
    ...
    const upProbability = directionProbability(candles, options?.macro);
