...
export default (w: Window) => {
  ...
  class MainPage extends w.HTMLElement {
    ...
    onconstructor(@Inject(StockService.SYMBOL) stockService: StockService, router: Router) {
      ...
      this.bookMarkCategoryDisplay(this.categories);
      this.bookMarkCategoryStockDisplay(this.selectedCategory, this.stocks);
      this.risingDisplay(this.rising);
      this.fallingStockDisplay(this.falling);
      // this.updateCards();
      // this.render()
    }
    ...
    @innerHtml('.falling-container .stock-list')
    fallingStockDisplay(falling: Stock[] = []) {
      return `
      ${falling
        .slice(0, 3)
        .map(s => `<swc-example-stock-stock-card data-id="${s.id}" class="card-item"></swc-example-stock-stock-card>`)
        .join('')}
        `;
    }
    @innerHtml('.bookmark-container .category-scroll')
    ...
    @innerHtml('.bookmark-container .stock-list')
    ...
    @innerHtml('.rising-container .stock-list')
    ...
    @innerHtml('.section:nth-child(2) .stock-list')
    ...
    render() {
      ...
      <div class="container">
        ...
        <div class="rising-container section">
        ...
        <div class="bookmark-container section">
        ...
        <div class="falling-container section">
