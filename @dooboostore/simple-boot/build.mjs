...
async function buildTarget(target, watch = false) {
  const baseOptions = {
    ...
    // external: ['reflect-metadata', '@dooboostore/core', '@dooboostore/core-node'], // Externalize dependencies
  };
  ...
  switch (target) {
    ...
    case 'cjs':
      ...
      const cjsPackageJson = { 
        name: '@dooboostore/simple-boot-cjs-internal',
        private: true,
        type: 'commonjs' 
      };
      fs.writeFileSync(
    ...
    case 'umd-bundle':
      ...
      await performBuild({
        ...
        outfile: path.resolve(__dirname, 'dist', 'umd-bundle', 'dooboostore-simple-boot.umd.js'),
        format: 'iife',
        globalName: 'dooboostoreSimpleBoot',
    ...
    case 'esm-bundle':
      ...
      await performBuild({
        ...baseOptions,
          bundle:true,
        entryPoints: [path.resolve(srcDir, 'bundle-entry.ts')],
        outfile: path.resolve(__dirname, 'dist', 'esm-bundle', 'dooboostore-simple-boot.esm.js'),
        format: 'esm',
