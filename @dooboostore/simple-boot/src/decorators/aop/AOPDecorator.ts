...
export const getAfters = (target: any): MetaDataPropertyAtomic<any, AOPOption | undefined>[] => {
  return (
    ObjectUtils.allProtoTypeName(target)
      .map(it => new MetaDataPropertyAtomic<any, AOPOption | undefined>(target, getAfter(target, it), it))
      .filter(it => it.metaData !== undefined) || []
  );
};

export const getProtoAfters = (
  target: any,
  propertyKey: string,
  type?: ConstructorType<any>
): MetaDataPropertyAtomic<any, AOPOption | undefined>[] => {
  return (
    getAfters(target).filter(it => propertyKey === it.metaData?.property && type === it.metaData.type?.prototype) || []
  );
};
...
export const getProtoBefores = (
  target: any,
  propertyKey: string,
  type?: ConstructorType<any>
): MetaDataPropertyAtomic<any, AOPOption | undefined>[] => {
  return (
    getBefores(target).filter(it => propertyKey === it.metaData?.property && type === it.metaData.type?.prototype) || []
  );
};
