...
import { ConstructorType, FieldType, FieldUnionType } from '@dooboostore/core/types';
...
export abstract class ComponentBase<T> implements OnChangeAttrRender, OnCreateRenderData, OnCreatedThisChild, OnDrThisBind, OnDrThisUnBind, OnInitRender, OnDestroyRender {
  ...
  getAttribute<K extends keyof T>(name: K): T[K] | null {
    const target = this._attribute as any;
    if (!target) {
      return null;
    }
  ...
  onCreateRenderData(data: OnCreateRenderDataParams): void {
    this._rawSet = data.render?.rawSet;
   this.setAttribute(data.render?.attribute as T);
  ...
  onInitRender(param: any, rawSet: RawSet) {
    if (rawSet) {
      this._rawSet = rawSet;
      if (this._rawSet) {
       this.setAttribute(rawSet.dataSet?.render?.attribute as T);
      }
