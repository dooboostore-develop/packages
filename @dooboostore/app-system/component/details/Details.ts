...
export namespace Details {
  ...
  @Component({
    template: '<form class="${`${@this@.attribute.class} details-form-container`}$" dr-on-init="@this@.$element = $element;">#innerHTML#</form>',
    styles: '.details-form-container {position: ${@this@.attribute.float ? "absolute" : "static"}$;}',
  ...
  export class Details extends ComponentBase<Attribute> implements OnCreateRender, OnInitRender {
    ...
    onCreateRenderData(data: OnCreateRenderDataParams) {
      ...
      // console.log('vvvvvvvvvvvvvv',  data.render?.getElement?.() as HTMLDetailsElement);
    }
    ...
    onInitRender(param: any, rawSet: RawSet): void {
      ...
      if (ValidUtils.isBrowser() && this.rawSet?.dataSet.config.window) {
        const detailsElement = this.getElement?.<HTMLDetailsElement>();
