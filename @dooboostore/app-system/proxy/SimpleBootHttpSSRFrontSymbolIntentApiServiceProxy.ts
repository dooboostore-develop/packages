      ...
      get(target: SimpleBootHttpSSRFrontSymbolIntentApiServiceProxy, prop: string | symbol, receiver: any) {
        ...
        if (typeof value === 'function') {
          return function(...args: any[]) {
            ...
            if (f.length >= 2) {
              const p = (param: any, userConfig?: {  bypassTransform?: boolean;transformText?: boolean} ) => {
                const fetchConfig = {config: config.config, fetch: {headers: makeIntentHeaderBySymbol(config.symbol), body: param ?? args[0]}};
                return config.apiService.postJson({
                  target: new URL(`/${String(prop)}`, config.host),
                  config: {...userConfig, ...fetchConfig}
                });
              };
              args.push(p);
            }
