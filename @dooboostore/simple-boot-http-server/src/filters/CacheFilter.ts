...
export class CacheFilter implements Filter {
  public static readonly SYMBOL = Symbol.for('CacheFilter');
  ...
  public async refresh(path: string, app: SimpleBootHttpServer) {
    // 1. 기존 캐시 파일 삭제
    this.clear(path);

    // 2. HTTP Fetch를 통한 외부 호출 (GET)
    // 아키텍트님의 지시에 따라 네트워크 레이어를 통해 정석대로 호출합니다.
    const url = `${app.option.protocol}://${app.option.hostname}:${app.option.port}${path}`;
    try {
      await fetch(url, {
        method: 'GET',
        headers: {
          'x-cache-force-refresh': 'true',
          'User-Agent': 'SimpleBootHttpServer-Internal-Refresh'
        }
      });
    } catch (e) {
      console.error('CacheFilter.refresh fetch error:', e);
    }
  }

  public clear(key: string) {
    this.initCacheConfig();
    const cache = this.cacheConfig.caches[key];
    if (cache) {
      this.deleteChunk(cache.path);
      delete this.cacheConfig.caches[key];
  ...
  public clearAll() {
    this.initCacheConfig();
    Object.keys(this.cacheConfig.caches).forEach(key => {
      this.deleteChunk(this.cacheConfig.caches[key].path);
    });
    this.cacheConfig.caches = {};
    this.saveCacheConfig();
  }

  private initCacheConfig() {
  ...
  private saveCacheConfig() {
    if (this.config.config.cacheDir && this.cacheConfig) {
