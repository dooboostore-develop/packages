...
// Function to find all .ts files in a directory, similar to the Webpack config
...
async function buildTarget(target, watch = false) {
  const baseOptions = {
    bundle: target.includes('bundle'),
    sourcemap: true,
    target: 'es2020',
    // The watch option is handled by performBuild, not passed directly here
  };
  ...
  switch (target) {
    ...
    case 'umd-bundle':
      ...
      await performBuild({
        ...
        outfile: path.resolve(__dirname, 'dist', 'umd-bundle', 'dooboostore-core-web.umd.js'),
        format: 'iife',
        globalName: 'dooboostoreCoreWeb',
    ...
    case 'esm-bundle':
      ...
      await performBuild({
        ...baseOptions,
        entryPoints: [path.resolve(srcDir, 'index.ts')],
        outfile: path.resolve(__dirname, 'dist', 'esm-bundle', 'dooboostore-core-web.esm.js'),
        format: 'esm',
        tsconfig: 'tsconfig.esm.json',
      }, watch);
